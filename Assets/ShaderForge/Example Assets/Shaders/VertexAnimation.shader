    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_8530 = _Time;
                float node_1924 = pow((abs((frac((o.uv0+node_8530.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape);
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_8530 = _Time;
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),pow((abs((frac((i.uv0+node_8530.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape)));
...
/////// Diffuse:
                ...
                float node_133 = pow((abs((frac((i.uv0+node_8530.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.tangentDir = normalize( mul( unity_ObjectToWorld, float4( v.tangent.xyz, 0.0 ) ).xyz );
                o.bitangentDir = normalize(cross(o.normalDir, o.tangentDir) * v.tangent.w);
                float4 node_4252 = _Time;
                float node_1924 = pow((abs((frac((o.uv0+node_4252.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape);
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                float3 lightColor = _LightColor0.rgb;
                o.pos = UnityObjectToClipPos( v.vertex );
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _Normals_var = UnpackNormal(tex2D(_Normals,TRANSFORM_TEX(i.uv0, _Normals)));
                float4 node_4252 = _Time;
                float3 normalLocal = normalize(lerp(_Normals_var.rgb,float3(0,0,1),pow((abs((frac((i.uv0+node_4252.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape)));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_8680 = _Time;
                float node_1924 = pow((abs((frac((o.uv0+node_8680.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape);
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
                o.pos = UnityObjectToClipPos( v.vertex );
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 node_2327 = _Time;
                float node_1924 = pow((abs((frac((o.uv0+node_2327.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape);
                v.vertex.xyz += (node_1924*_BulgeScale*v.normal);
                o.posWorld = mul(unity_ObjectToWorld, v.vertex);
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 node_2327 = _Time;
                float node_1924 = pow((abs((frac((i.uv0+node_2327.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape);
                o.Emission = (_GlowColor.rgb*_GlowIntensity*node_1924);
                
                float4 _Diffuse_var = tex2D(_Diffuse,TRANSFORM_TEX(i.uv0, _Diffuse));
                float node_8608 = 0.1;
                float3 diffColor = lerp(_Diffuse_var.rgb,float3(node_8608,node_8608,node_8608),pow((abs((frac((i.uv0+node_2327.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape));
                float node_4921 = 1.0;
